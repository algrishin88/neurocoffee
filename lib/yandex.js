...
const getAuthorizationUrl = () => {
  ...
  // scope нужен для доступа к email и данным профиля (login.yandex.ru/info)
...
const getAccessToken = async (code) => {
  ...
  try {
    ...
    console.error('❌ Ошибка получения токена Яндекса:', { status, data, message: error.message });
    if (data?.error === 'invalid_grant' || (msg && String(msg).toLowerCase().includes('redirect'))) {
      throw new Error('Код авторизации недействителен или истёк. В кабинете Яндекс OAuth (oauth.yandex.ru) в Callback URI должно быть ровно: ' + YANDEX_REDIRECT_URI);
    }
    if (data?.error === 'unauthorized_client') {
      throw new Error('Неверный client_id или client_secret. Проверьте YANDEX_CLIENT_ID и YANDEX_CLIENT_SECRET в настройках сервера.');
    }
